...
########### Drop Date #####################
# Loop through each country with date excluded as a random effect
for (cntry in countries) {
  ...
  # Fit RE-Only Model without date
  re_only_model <- glmer.nb(
    count ~ 1 + 
      (1 | site),
    data = country_data
  ...
  # Fit Full Model with Treatment, without date
  treatment_model <- glmer.nb(
    count ~ treatment + 
      (1 | site),
    data = country_data
  ...
  cat(paste0("####################### COUNTRY: ", toupper(cntry), " (drop_date) #######################\n"),
      file = summary_file, append = TRUE)
  cat(paste0("################ RE-ONLY MODEL SUMMARY (", cntry, ", drop_date) ################\n"),
      file = summary_file, append = TRUE)
  capture.output(print(summary(re_only_model)),
                 file = summary_file, append = TRUE)
  cat(paste0("\n################ FULL MODEL SUMMARY (", cntry, ", drop_date) ################\n"),
      file = summary_file, append = TRUE)
  capture.output(print(summary(treatment_model)),
                 file = summary_file, append = TRUE)
  cat("\n\n", file = summary_file, append = TRUE)
  ...
  # Plot residuals vs fitted
  residual_vs_fitted_path <- file.path(residual_dir, paste0(cntry, "_residuals_vs_fitted_drop_date.png"))
  png(residual_vs_fitted_path)
  plot(fitted(treatment_model), treatment_residuals, 
  ...
  # Q-Q plot
  qq_plot_path <- file.path(residual_dir, paste0(cntry, "_qq_plot_drop_date.png"))
  png(qq_plot_path)
  qqnorm(treatment_residuals, 
         main = paste("Q-Q Plot of Residuals for", eco_function, "in", cntry, "(drop_date)"))
  qqline(treatment_residuals, col = "red")
  dev.off()
}

...
# Path to our new CSV for per-country FE results
...
###############################################################################
# Function to capture & append FE results
###############################################################################
capture_fe_results <- function(model, eco_fun, model_name, country_name, coefs_log, coefs_exp, coefs_raw) {
...
###############################################################################
# Modified print_model_results that also captures FE
###############################################################################
print_model_results <- function(model, model_name, country_name) {
  ...
  # If it's the FE model (i.e. "Full Model with Treatment"), append to CSV
  if (!grepl("RE-Only", model_name, ignore.case = TRUE)) {
    capture_fe_results(
      model, eco_function, model_name, country_name,
...
# Empty the summary file (if it exists already)
...
for (cntry in countries) {
  ...
  # Append summaries to the main file
...
message("Done. Fixed-effects model results appended to country_combined_results.csv (skipping RE-only).")

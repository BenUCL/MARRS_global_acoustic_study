...
# -------------------------
# Input CSV (e.g., phonic_richness_age.csv)
...
data$treatment_new <- factor(data$treatment_new, levels = c("degraded", "healthy", "restoration_intervention"))
...
# For healthy and degraded rows, set age to 0 (so the smooth term gets multiplied by 0)
...
# Fit the GAM Model
# -------------------------
# For phonic_richness, no offset is applied.
gam_model <- gam(
  count ~ treatment_new +
...
# -------------------------
# Extract smooth terms table from model summary

...
# -------------------------
# Input CSV for snaps_count (e.g., snaps_count_age.csv)
csv_path <- file.path(BASE_DIR, "marrs_acoustics/data/results/functions", paste0(eco_function, ".csv"))
# Output folder for GAM stats for snaps_count
...
data$date      <- as.factor(data$date)  # use factor for random effects
...
# For healthy and degraded rows, set age to 0 (so the smooth term is multiplied by 0)
...
# Fit the GAM Model
# -------------------------
# For snaps_count, no offset is used.
gam_model <- gam(
  count ~ treatment_new +
...
summary_file <- file.path(out_folder, "snaps_count_gam_summary.txt")
sink(summary_file)
...
pred_data <- data.frame(
  ...
  age = age_seq,
  # For random effects, set arbitrary valid levels (they will be excluded in prediction)
...
p <- ggplot(pred_data, aes(x = age, y = fit_resp)) +
  ...
  labs(title = "Effect of Age on Count for Restoration Intervention (snaps_count)",
...
# Create the same plot with a log10 y-axis
